...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-[356px] overflow-hidden rounded-xl border border-trueme-gold/20 bg-white shadow-sm",
        className
      )}
      {...props}
    />
...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
        "flex flex-col space-y-1.5 border-t border-trueme-gold/20 p-4",
        className
      )}
      {...props}
    />
...
export function CardTitle({ className, ...props }: CardTitleProps) {
  return (
    <h3
      className={cn(
        "text-lg font-semibold leading-none tracking-tight text-trueme",
        className
      )}
      {...props}
    />
...
const Layer1: React.FC<LayerProps> = ({ color, secondaryColor }) => {
  return (
    ...
    >
      <div className="flex shrink-0 items-center rounded-full border border-trueme-gold/30 bg-white/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--color)]" />
        <span className="ml-1 text-[10px] text-trueme">
          +15,2%
        </span>
      </div>
      <div className="flex shrink-0 items-center rounded-full border border-trueme-gold/30 bg-white/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
  ...
  return (
    ...
    >
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex w-[356px] translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
        <div className="ease-[cubic-bezier(0.6, 0, 1)] rounded-md border border-trueme-gold/30 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100">
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 shrink-0 rounded-full bg-[var(--color)]" />
            <p className="text-xs text-trueme">
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  ...
  return (
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[8] flex h-[180px] w-[356px] items-center justify-center text-trueme-gold/20 transition-transform duration-500 group-hover/animated-card:scale-150">
