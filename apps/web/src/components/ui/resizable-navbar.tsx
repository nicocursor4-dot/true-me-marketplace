...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={cn("sticky inset-x-0 top-2 z-[60000] w-full", className)}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex dark:bg-transparent transition-colors duration-300 ease-out",
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <a
          onClick={onItemClick}
          className="relative px-4 py-2 text-neutral-600 dark:text-neutral-300 hover:text-trueme-gold transition-colors duration-200"
          key={`link-${idx}`}
          href={item.link}
        >
