...
function TierBanner() {
  ...
  const selectedTierLabel = useMemo(() => {
    switch ((selectedTier || '').toLowerCase()) {
      case 'express': return "Authentification Express"
      case 'complete': return "Authentification Complète"
      case 'premium': return "Authentification Premium"
      default: return null
    }
  }, [selectedTier])

  if (!selectedTierLabel) return null

  return (
    <div className="rounded-2xl border border-trueme-gold/30 bg-trueme-gold/10 text-trueme p-4 -mt-8">
      <div className="flex items-center justify-between gap-4">
        <p className="font-medium">Plan sélectionné: {selectedTierLabel}</p>
        <Link href="/authentification#plans" className="text-trueme underline underline-offset-4 hover:text-trueme-gold">Modifier</Link>
      </div>
    </div>
  )
}
          ...
          <div className="glass-morphism p-8 rounded-2xl space-y-6">
            <label className="flex items-start space-x-4">
              ...
              <span className="text-lg text-trueme-light leading-relaxed">
                J'accepte les{' '}
                <Link href="/legal/conditions" className="text-trueme-gold font-semibold hover:text-trueme luxury-hover">
                  conditions d'utilisation
                </Link>
                {' '}et la{' '}
                <Link href="/legal/confidentialite" className="text-trueme-gold font-semibold hover:text-trueme luxury-hover">
